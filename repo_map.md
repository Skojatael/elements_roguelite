...
- **methods**: `get_dungeon_config() -> Dictionary`, `get_meta_config() -> Dictionary`, `get_relics() -> Dictionary`, `get_skills() -> Array`, `get_enemy_data(id: String) -> Dictionary`, `get_enemy_base_essence(id: String) -> float`, `get_enemy_rooms_required(id: String) -> int`, `enemy_id_exists(id: String) -> bool`, `get_combat_room_pool(domain: String) -> Array`
...
- **state**: `run_id: String`, `is_run_active: bool`, `run_mode: String`, `run_domain: String`, `current_tier: int`, `run_start_time: float`, `run_currency: float`, `current_room: Node`, `cleared_rooms: Dictionary`, `enemies_slain: int`, `run_state: RunState`, `run_summary: RunSummary`, `player_state: PlayerState`
- **services** (stubs): `difficulty_service: DifficultyService`, `rewards_service: RewardsService`
- **methods**: `start_run(mode: String, domain: String = "forest")`, `end_run(reason: EndReason)`, `spawn_room(room_data, room_id, context) -> RoomSpawner`, `register_room(spawner: RoomSpawner)`, `add_currency(amount: float)`, `mark_room_cleared(room_id: String)`, `is_room_cleared(room_id: String) -> bool`
...
- **fields**: `id`, `display_name`, `max_health`, `damage`, `move_speed`, `detection_range`, `damage_cooldown`, `base_essence`, `rooms_required: int`, `damage_reduction: float`, `reflect_amount: float` (0.0 default; used by player Thorn Bark relic only — enemy entries keep this at 0.0), `root_duration: float`, `root_cooldown: float`, `poison_duration: float` (0.0 default; > 0 means enemy can apply poison on contact), `poison_modifier: float` (0.0 default; fraction damage reduction applied to poisoned target), `regen_rate: float` (0.0 default; fraction of max HP recovered per second when > 0), `heal_amount: float` (0.0 default; flat HP granted to each in-radius ally per skill use when > 0), `heal_radius: float` (0.0 default; px radius within which allies are healed), `heal_cooldown: float` (5.0 default; seconds between heal skill activations), `buff_zone_radius: float` (0.0 default; world-unit radius of the buff zone; > 0 activates buffer behaviour), `buff_cooldown: float` (0.0 default; seconds between zone casts), `buff_zone_duration: float` (0.0 default; seconds the zone persists), `buff_regen_rate: float` (0.0 default; fraction of max HP per second granted to enemies inside zone), `buff_attack_speed_bonus: float` (0.0 default; additive attack speed multiplier bonus granted to enemies inside zone), `size: float` (1.0 default; uniform scale multiplier applied against `base_size` from dungeon_config to set visual size and body collision radius), `charge_attack_damage: float` (0.0 default; damage dealt on charge contact; 0.0 = no charge attack), `charge_attack_cooldown: float` (0.0 default; seconds between charges; 0.0 = no charge attack), `charge_attack_length: float` (0.0 default; px distance traveled per charge; also sets telegraph rectangle long axis), `shield_hp: int` (0 default; maximum shield HP; 0 = no shield capability), `shield_stun_duration: float` (3.0 default; seconds enemy is stunned when shield breaks), `scene_path: String` ("" default; if non-empty RoomSpawner loads this scene instead of Enemy.tscn — used for boss enemies with dedicated scripts), `thorns_on_hit: bool` (false default; when true enemy fires thorn projectiles in all configured directions when hit), `thorns_damage: float` (0.0 default; damage per thorn projectile), `thorns_speed: float` (400.0 default; thorn projectile travel speed px/s), `thorns_range: float` (600.0 default; max thorn travel distance before despawn), `thorns_fire_cooldown: float` (0.5 default; minimum seconds between thorn bursts), `thorns_directions: int` (4 default; 4 = diagonal NE/NW/SE/SW, 6 = adds N/S), `thorns_duration: float` (3.0 default), `thorns_cooldown_p2: float` (10.0 default), `thorns_cooldown_p3: float` (6.0 default), `recover_duration: float` (0.6 default; seconds boss pauses after a charge)
- **factory**: `static func from_dict(data) -> EnemyData`
- **static**: `static func is_ranged_attacker(attack_range: float, threshold: float) -> bool` — returns `attack_range > threshold`; determines if an enemy should fire projectiles instead of melee contact damage

### `scripts/data_models/MetaState.gd` (`class_name MetaState extends RefCounted`)
- **fields**: `total_shards: int`, `damage_upgrade_level: int`, `relic_offers_active: bool`, `first_boss_killed: bool`, `adventuring_gear_owned: bool`, `endless_boss_kill_count: int`, `boss_run_unlocked: bool`, `magic_forge_unlocked: bool`, `mage_tower_unlocked: bool`, `alchemy_lab_unlocked: bool`, `essence_gain_level: int`, `gold_generator_owned: bool`, `gold_storage_cap_level: int`, `total_gold: float`, `gold_last_saved_timestamp: int`, `shard_generator_level: int`, `shard_accumulator: float`, `missile_extra_charge_owned: bool`, `rarity_luck_owned: bool`, `book_of_skill_gate_reached: bool`, `book_of_skill_owned: bool`, `forest_domain_unlocked: bool`, `depth_scaling_unlocked: bool`
...
- **fields**: `room_id: String`, `spawn_points: Array[SpawnPointData]`, `enemy_count_mult: float`, `essence_mult: float`, `wave_config: WaveConfig`, `wave_spawn_points: Array` (array of `Array[SpawnPointData]` indexed by wave; populated by `RoomSpawner._load_depth_band_config()` for combat rooms and `_load_elite_depth_band_config()` for elite rooms)
...
- **methods**: `_generate()`, `_generate_with(config: Dictionary, gear_owned: bool, depth_scaling: bool = false, domain: String = "forest")`, `_record_room(cell, type_id, occupied, frontier, difficulty_scale, depth_scaling: bool = false)`, `_build_neighbours(occupied)`, `_promote_elite_rooms()`, `_expand_dungeon(occupied, pool, difficulty_scale, depth_scaling: bool = false)`, `_get_expansion_neighbours(cell, occupied, min_depth) -> Array[Vector2i]`, `_get_valid_neighbours(cell, occupied) -> Array[Vector2i]`, `_get_world_pos(cell) -> Vector2`
...
- **routing**: `_load_config()` routes combat rooms (those in `combat_room_pool`) to `_load_depth_band_config()`; rooms whose `room_type_id` contains `"Elite"` to `_load_elite_depth_band_config()`; all others use `spawn_configs`
- **static method**: `pick_elite_variant(bands: Array, p_depth: int) -> Dictionary` — pure band matching + weighted variant selection from `elite_depth_bands`; returns selected variant dict or empty dict on no match
- **methods**: `_load_depth_band_config(raw: Dictionary) -> RoomSpawnConfig`, `_load_elite_depth_band_config(raw: Dictionary) -> RoomSpawnConfig` — build `wave_spawn_points` and minimal `wave_config` for band-spawned rooms
- **methods**: `_resolve_wave_config()`, `_spawn_wave(wave_idx: int)`, `_spawn_enemies_legacy()`, `_lock_doors()`, `_unlock_doors()`, `_get_scene_for_enemy(enemy_id: String) -> PackedScene` — returns a scene loaded from `EnemyData.scene_path` if set, otherwise `ENEMY_SCENE`; used by all three spawn paths
...
- **properties**: `damage_reduction: float` (0.0 default; recomputed from relics on `relic_applied`/`relics_cleared` for player; set from `EnemyData` for enemies); `reflect_amount: float` (0.0 default; recomputed from `RelicManager.get_stat_addend("reflect_amount")` for player; set from `EnemyData` for enemies); `is_invulnerable: bool` (set true by DodgeComponent at dash start; cleared at dash end; guards both `take_damage` and `take_damage_raw`)
- **methods**: `take_damage(amount: float, attacker: StatsComponent = null)`, `take_damage_raw(amount: float)`, `heal(amount: float)`, `reset()`
...
- **notes**: `_process(delta)` ticks HP regen when `is_player`, `RunManager.is_run_active`, and `RelicManager.get_stat_addend("hp_regen") > 0`; `take_damage_raw()` bypasses DR (used for burn DoT); both damage methods return early when `is_invulnerable`; `take_damage()` fires reflect to `attacker` (if non-null and `reflect_amount > 0`) via `attacker.take_damage(floori(effective * reflect_amount))` — no attacker arg on that call prevents chain
...
- **methods**: `setup(target: Enemy, damage: float, speed: float, max_distance: float, chain_damage_mult: float, burn_damage_per_tick: float, burn_duration: float, burn_extend_seconds: float, attacker_stats: StatsComponent = null)` — initialises homing target, damage, speed, max travel distance, chain multiplier, burn params, attacker's StatsComponent (for reflect) and connects hit collision; `_try_chain(primary_target: Enemy)` — if `chaining_stone` relic held, applies `_damage * (_chain_damage_mult + RelicManager.get_chain_damage_bonus())` to closest other living enemy; passes `_attacker_stats` to all `take_damage()` calls

### `scenes/combat/enemies/Enemy.gd`
- **const**: `DETECTION_RANGE_FALLBACK = 300.0`; `THORNS_DIRS_4: Array[Vector2]` — four diagonal unit vectors (NE, NW, SE, SW); `THORNS_DIRS_6: Array[Vector2]` — THORNS_DIRS_4 plus N and S
...
- **fields**: `_spawn_delay: float` — per-instance countdown read from `dungeon_config.json → enemy_spawn.spawn_delay`; blocks movement and contact damage until it reaches 0; `_root_cooldown_remaining: float` — per-instance cooldown between successive roots applied to the player; `_heal_cooldown_remaining: float` — countdown timer for the ally-heal skill; initialised to `data.heal_cooldown` in `initialize()`; `_follow_target: Enemy` — frame-cache of the closest living ally sibling; resolved each physics frame when enemy is a healer; null when no allies present; `_root: RootComponent` — child node instantiated in `_ready()`; tracks root status applied to this enemy by the player; `_poison: PoisonComponent` — child node instantiated in `_ready()`; tracks poison status applied to this enemy by the player relic; `_buff_cooldown_remaining: float` — countdown to next buff zone cast; initialised to `data.buff_cooldown` in `initialize()`; only ticked when `_data.buff_cooldown > 0`; `_zone_regen_rate: float` — accumulated regen bonus from all overlapping buff zones; `_zone_attack_speed_bonus: float` — accumulated attack speed bonus from all overlapping buff zones; `_is_ranged: bool` — set once in `_ready()` via `EnemyData.is_ranged_attacker()`; when true, contact-damage tick fires a projectile instead of applying direct damage; visual size and body collision radius computed in `initialize()` as `base_size × data.size` (base_size from `dungeon_config.json → enemy_spawn.base_size`); `_charge_cooldown_remaining: float` — countdown to next charge; starts at `data.charge_attack_cooldown` on player detection; only active when `charge_attack_cooldown > 0`; `_telegraph_timer: float` — elapsed seconds in TELEGRAPHING state; charge fires when ≥ 2.0; `_charge_direction: Vector2` — unit vector locked at telegraph start toward player position; `_charge_distance_remaining: float` — px remaining in current charge; enemy stops when ≤ 0; `_telegraph_node: Node2D` — semi-transparent rectangle added to room parent during telegraph/charge phases; freed on charge completion or cancel; `_charge_hit_delivered: bool` — one-shot guard preventing multiple damage applications per charge; `_current_shield_hp: int` — runtime shield HP; 0 when shield inactive; absorbs damage before regular HP when > 0; `_stun_remaining: float` — countdown timer for post-shield-break stun; 0 when not stunned; `_shield_visual: ColorRect` — semi-transparent blue overlay shown while shield is active; created programmatically in `initialize()`; swap for sprite in a future feature
- **signals**: `defeated`
- **enum**: `EnemyState { IDLE, PURSUING, TELEGRAPHING, CHARGING, STUNNED }`
- **methods**: `initialize(data: EnemyData)`, `apply_difficulty(mult: float)` — scales max/current health and emits `health_changed` so connected HP bars update immediately, `get_hp_ratio() -> float`, `is_burning() -> bool`, `activate_shield() -> void` — sets `_current_shield_hp` to `_data.shield_hp` and shows shield visual; no-op when `shield_hp = 0`, `take_damage(amount: float, attacker: StatsComponent = null)` — intercepts shield HP first; overflow carries to regular HP; on shield break calls `_on_shield_broken()`; after damage resolves calls `_try_fire_thorns()`, `receive_heal(amount: float) -> void` — restores regular HP only; shield HP is intentionally unaffected, `apply_zone_buff(regen: float, speed: float) -> void` — adds regen and speed values to zone accumulators; called by EnemyBuffZone on body_entered, `remove_zone_buff(regen: float, speed: float) -> void` — subtracts from accumulators clamped to 0; called by EnemyBuffZone on body_exited and zone expiry, `on_burn_hit(tick_dmg: float, base_duration: float, extend_seconds: float)`, `apply_root(duration: float) -> void` — applies root to this enemy (refresh-to-longest); called by CombatComponent when root relic procs; `apply_poison(duration: float, modifier: float) -> void` — applies poison to this enemy (stacks duration); called by RelicManagerImpl.try_apply_poison via CombatComponent; `_fire_thorns() -> void` — inherited by ForestBossThorns; spawns one EnemyProjectile per direction in the selected direction set

### `scenes/combat/enemies/ForestBossThorns.gd` (`class_name ForestBossThorns extends Enemy`)
- **const**: `PHASE2_THRESHOLD = 0.667`, `PHASE3_THRESHOLD = 0.333`
- **enum**: `BossState { IDLE, CHASE, WINDUP_CHARGE, CHARGING, RECOVER, THORNS_ACTIVE, PHASE_TRANSITION, STUNNED, DEAD }`
- **fields**: `_boss_state: BossState`, `_phase: int` (1/2/3), `_phase2_triggered: bool`, `_phase3_triggered: bool`, `_charge_cooldown: float`, `_telegraph_timer: float`, `_recover_timer: float`, `_transition_timer: float`, `_thorns_timer: float`, `_thorns_cooldown_remaining: float`, `_charge_direction: Vector2`, `_charge_distance_remaining: float`, `_charge_hit_delivered: bool`, `_telegraph_node: Node2D`, `_player_stats_boss: StatsComponent`
- **signals**: `defeated` (inherited from Enemy)
- **methods** (overrides): `_ready()` — calls `super._ready()` then `activate_shield()`; `_on_detected(body)` → CHASE; `_on_lost(body)` → IDLE; `_on_shield_broken()` → STUNNED; `_on_died()` → DEAD, emits defeated, queue_free; `take_damage(amount, attacker)` — calls super then fires `_fire_thorns()` (inherited) when in THORNS_ACTIVE and fire cooldown permits
- **methods** (boss-specific): `_check_phase_transition()` — checks HP ratio against thresholds, sets guard flags, increments `_phase`, starts thorns cooldown, enters PHASE_TRANSITION; `_enter_phase_transition()`, `_start_windup_charge()`, `_cancel_charge_boss()`, `_enter_thorns_active()`, `_exit_thorns_active()`

### `scenes/combat/enemies/EnemyProjectile.gd` (`class_name EnemyProjectile extends Node2D`)
- **exports**: `_hit_area: Area2D` — assigned in Inspector; connected to `_on_body_entered` in `setup()`
- **const**: `MAX_RANGE: float = 1200.0`
- **fields**: `_direction: Vector2`, `_damage: float`, `_speed: float`, `_max_range: float`, `_distance_traveled: float = 0.0`
- **methods**: `setup(direction: Vector2, damage: float, speed: float, max_range: float) -> void` — normalises direction, stores all params, connects hit_area signal; speed and max_range read from `dungeon_config.json → enemy_spawn` by caller; `_on_body_entered(body: Node2D) -> void` — early-returns if body not in group `"player"`; otherwise calls `StatsComponent.take_damage()` and `queue_free()`

### `scenes/combat/enemies/EnemyBuffZone.gd` (`class_name EnemyBuffZone extends Area2D`)
- **const**: `ZONE_COLOR = Color(0.4, 0.8, 1.0, 0.25)`
- **exports**: `_collision_shape: CollisionShape2D`, `_visual: ColorRect`
- **fields**: `_regen_rate: float`, `_attack_speed_bonus: float`, `_duration_remaining: float`, `_buffed_enemies: Array[Enemy]`
- **methods**: `setup(radius: float, duration: float, regen_rate: float, attack_speed_bonus: float) -> void` — sizes collision shape and visual, stores buff values; must be called after `add_child`
...
- **exports**: `_de_button: Button`, `_rs_button: Button`, `_bc_button: Button`, `_ds_button: Button`, `_close_button: Button`
...
- **signals**: `close_pressed`
- **exports**: `_close_button: Button`, `_forest_label: Label`, `_forest_button: Button`
- **methods**: `_update_ui()` *(reads forest_domain cost from meta_config, updates label text and button disabled/text state based on MetaManager.is_forest_domain_unlocked and can_spend)*
...
- **signals**: `hub_exited(domain: String)`, `hub_boss_run_pressed`
...
- **signals**: `teleport_activated(domain: String)`
- **exports**: `button: Button`, `domain: String = "forest"`
